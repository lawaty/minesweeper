...
class PWM:
    __slots__ = ["__pca", "__pin", "__min", "__max", "__logger", "__die_on_range_err"]

    @throws(ValueError, PCAConnectionError)
    def __init__(self, pin: int, config: tuple, die_on_range_err: bool = False):
        self.__pin = pin
    ...
    def __map(self, val: int) -> int:
        if val < self.__min or val > self.__max:
            exception = OutOfRangeSignal(
                f"PWM Value must be between {self.__min} and {self.__max} but found {val}"
            )

            if self.__die_on_range_err:
                raise exception
            else:
                self.__logger.logE(exception)
